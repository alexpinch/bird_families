...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_guess, {
    ...
    if (input$bird_search == answer) {
      values$game_over <- TRUE
    } else {
      # Reset bird_info to NULL if not game over
      values$bird_selected <- NULL
    }
  })
  
  output$bird_info <- renderUI({
    selected_bird <- input$bird_search
    req(input$submit_guess) # Wait for submit_guess button click
  ...
  output$guess_count <- renderText({
    ...
    paste("Guesses used:", values$guesses)
  })
}
